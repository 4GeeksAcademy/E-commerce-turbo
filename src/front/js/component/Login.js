...
function Login({ onLoginSuccess, onBackToApp }) {
  ...
  const handleResetPassword = async (e) => {
    e.preventDefault();
    console.log(process.env.BACKEND_URL+ 'api/solicitar-restablecimiento');
    console.log(formData.email);
    setError('');
    setIsLoading(true);
    try {
      const response = await fetch(process.env.BACKEND_URL + 'api/solicitar-restablecimiento', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      if (response.ok) {
        ...
        const errorData = await response.json();
        setError(errorData.msg || 'Error al solicitar el restablecimiento de contraseña. Por favor, intente de nuevo.');
      }
    } catch (error) {
      console.error('Error al enviar la solicitud:', error);
      setError('Error de conexión. Por favor, intente de nuevo más tarde.');
